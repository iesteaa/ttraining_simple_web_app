...
2. Copy the example environment files into place before reopening the container:
...
4. Wait until container build and setup commands complete.
5. In the dev container terminal:
...
6. Confirm runtime services are visible:
  - `backend` is running
  - `frontend` is running
  - `db` is healthy
  - `db_test` is healthy
...
If Docker Desktop is closed or still starting, open it first before reopening the dev container. See [Docker Daemon Not Ready For Dev Container](../docs/docker-daemon-devcontainer.md).
...
- Missing `.env` or `.env.test` during post-create setup:
  - Copy the example files into the workspace root before reopening the container.
